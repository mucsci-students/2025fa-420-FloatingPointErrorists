...
def add_times(default: bool) -> dict[str, list[str]]:
    ...
    def add_day() -> None:
        ...
        time_ranges_input = click.prompt(
            "Time ranges for this day (e.g., 9-11, 13-15 or 09:00-11:00)"
        )
        time_ranges = [
            normalize_range(r) for r in time_ranges_input.split(",") if r.strip()
        ]
...
def add_course_preferences(json_config: JsonConfig, default: bool) -> dict[str, int]:
    ...
    while click.confirm("Add course preference?", default=default):
        ...
        preference = click.prompt(
            "Preference score", type=click.IntRange(min=0, max=10)
        )
...
def add(ctx: click.Context) -> None:
    ...
    minimum_credits = click.prompt(
        "Minimum credit hours",
        type=click.IntRange(min=0, max=maximum_credits),
        default=maximum_credits,
    )
    unique_course_limit = click.prompt(
...
def modify(ctx: click.Context) -> None:
    ...
    maximum_credits = click.prompt(
        "Maximum credit hours",
        type=click.IntRange(min=0),
        default=faculty_obj.maximum_credits,
    )
    minimum_credits = click.prompt(
        "Minimum credit hours",
        type=click.IntRange(min=0, max=maximum_credits),
        default=faculty_obj.minimum_credits
        if maximum_credits == faculty_obj.maximum_credits
        else maximum_credits,
    )
    unique_course_limit = click.prompt(
        "Unique course limit",
        type=click.IntRange(min=1),
        default=faculty_obj.unique_course_limit,
    )
    times = faculty_obj.times
    if click.confirm(
        "Modify available times? (you will create a new set from scratch)",
        default=False,
    ):
    ...
    if click.confirm(
        "Modify lab preferences? (you will create a new one from scratch)",
        default=False,
    ):
