...
def add_time_block(ctx: click.Context) -> None:
    ...
    click.echo(
        TimeSlot.add_time_block(json_config, day_index, start, spacing, end)
    )
...
def add_class_pattern(ctx: click.Context) -> None:
    ...
    click.echo(
        TimeSlot.add_class_pattern(json_config=json_config, creds=creds, meetings=meetings, disabled=disabled, start_time=cp_start_time)
    )
...
def delete_time_block(ctx: click.Context) -> None:
    """Delete a time block."""
    json_config = get_json_config(ctx)
    day = click.prompt("Enter the day to delete from", type=click.Choice(DAYS, case_sensitive=False)).upper()
    day_index = DAY_TO_INDEX[day]
...
def modify_time_block(ctx: click.Context) -> None:
    """Modify a time block."""
    json_config = get_json_config(ctx)
    day = click.prompt("Enter the day to modify from", type=click.Choice(DAYS, case_sensitive=False))
    day_index = DAY_TO_INDEX[day]
    if len(json_config.time_slot_config.times.get(day)) == 0:
...
def modify_class_pattern(ctx: click.Context) -> None:
    ...
    json_config = get_json_config(ctx)
    index = click.prompt("Enter the number class pattern to modify", type=click.IntRange(0, len(json_config.time_slot_config.classes) - 1))
    old_time_slot = json_config.time_slot_config.classes[index]
    creds = click.prompt("Credits", type=int)
    meetings: list[Meeting] = []

    def add_meeting() -> None:
        meeting_day = click.prompt("Meeting day", type=click.Choice(DAYS, case_sensitive=False)).upper()
        start_time = normalize_time(click.prompt("Start time (e.g., 9 or 09:00)"))
        duration = click.prompt("Duration (minutes)", type=click.IntRange(min=0))
        lab = click.confirm("Is this a lab meeting?", default=False)
        meeting = Meeting(day=meeting_day, start_time=start_time, duration=duration, lab=lab)
