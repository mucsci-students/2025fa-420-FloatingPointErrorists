...
class TimeSlot:
    ...
    @staticmethod
    def add_time_block (json_config: JsonConfig, day_index:int, start:TimeString, spacing:int, end:TimeString) -> str:
    ...
    @staticmethod
    def add_class_pattern(json_config: JsonConfig, creds:int, meetings:list[Meeting], disabled:bool, start_time:TimeString|None) -> str:
        """ Adds a class pattern to the config """
        class_pattern = ClassPattern(credits=creds, meetings=meetings, disabled=disabled, start_time=start_time)
        json_config.time_slot_config.classes.append(class_pattern)
        return "Class pattern added successfully."

    @staticmethod
    def mod_time_block(day_index:int , index: int, json_config: JsonConfig, start:TimeString, spacing:int, end:TimeString) -> str:
    ...
    @staticmethod
    def mod_class_pattern(index: int, json_config: JsonConfig, creds: int, meetings: list[Meeting], disabled: bool, start_time: TimeString|None) -> str:
        ...
        class_pattern = ClassPattern(
            ...
            start_time=start_time if start_time is not None else old_class_pattern.start_time
        )
